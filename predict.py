...
def draw(filename, result):
    ...
    for class_name, lx, ly, rx, ry, ang, prob in res:
        ...
        # print(x0n, y0n, x1n, y1n)
        # blue  横线
        draw.line([(x0n, y0n), (x1n, y1n)], fill=(0, 0, 255), width=5)
        # red    竖线
        draw.line([(x1n, y1n), (x2n, y2n)], fill=(255, 0, 0), width=5)
    ...
    plt.show()
    # img.save(
    #     os.path.join('img_ret', 'dla_dcn_34_best_v2',
...
def pre_process(image):
    ...
    (mean, std) = cv2.meanStdDev(inp_image)
    mean = np.array(mean / 255, dtype=np.float32).squeeze().reshape(1, 1, 3)
    std = np.array(std / 255, dtype=np.float32).squeeze().reshape(1, 1, 3)
    # print(np.array(mean / 255, dtype=np.float32).squeeze().reshape(1, 1, 3))
...
def merge_outputs(detections):
    ...
    max_obj_per_img = 100
    scores = np.hstack(
        [detections[j][:, 5] for j in range(1, num_classes + 1)])
