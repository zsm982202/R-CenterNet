...
class ctDataset(data.Dataset):
    ...
    def __init__(self, data_dir='data', split='train'):
        self.data_dir = os.path.join(data_dir, 'weld')
    ...
    def __getitem__(self, index):
        ...
        (mean, std) = cv2.meanStdDev(inp)
        self.mean = np.array(mean, dtype=np.float32).squeeze().reshape(1, 1, 3)
        self.std = np.array(std, dtype=np.float32).squeeze().reshape(1, 1, 3)
        ...
        for k in range(num_objs):  # num_objs图中标记物数目
            ...
            bbox[:2] = affine_transform(bbox[:2],
                                        trans_output)  # 将box坐标转换到 128*128内的坐标
